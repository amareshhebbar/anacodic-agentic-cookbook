...
- **Multi-agent orchestration** (a supervisor routing to specialist agents,
  agents-as-tools). That's generation and domain judgment, not machinery, and
  it stays out of this cookbook. Note this is narrower than it used to read:
  *single-agent* orchestration is now in scope and built — see the forward
  pointers below.
...
## What decides when to use any of this

Everything in this stage runs in a fixed order every time: `08-backend-cascade`
tries its sources in the same sequence regardless of the question, and
`09-multi-query` runs every phrasing it is handed up front rather than trying a
second one *because* the first came back weak. That is a pipeline, and it is
the right shape for a tool.

The layer that turns these into decisions lives outside this stage:

- [`01-tools/07-as-a-tool`](../07-as-a-tool/) — collapses this whole stage into
  one `search_documents()` call an agent can choose to make, and adds a second
  tool so there is a choice at all.
- [`04-orchestrate`](../../04-orchestrate/) — decides whether to retrieve,
  which source to try first, and whether a weak grounding verdict warrants a
