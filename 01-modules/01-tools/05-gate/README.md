...
## Who acts on the verdict

`check_grounding()` returns a verdict; on its own it is a **report**, not a
trigger — nothing in this stage retries, refuses, or changes course because of
what it found.

Two stages consume it:

- [`04-orchestrate/03-retry-on-verdict.ipynb`](../../04-orchestrate/) — the
  first thing in this repo that *acts* on a weak verdict, retrying with a
  reformulated query under a mandatory cap.
- [`03-guardrails`](../../03-guardrails/) — the neighbouring, deliberately
  separate layer. Keep the boundary straight: **this stage judges evidence
  that came back; `03-guardrails` stops an action before it happens.** A
  grounding check inspects a produced answer. A guardrail blocks an input or a
  tool call from running at all.
